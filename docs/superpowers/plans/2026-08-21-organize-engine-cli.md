...
| §5.5 `move_folders` | **계획 3** | 기본이 꺼짐이고, 폴더를 옮기는 유일한 기능이라 따로 다룬다. **독립 실행이 기본 쓰임새** — 다른 작업 사이에 끼우지 않는다 |
| §5.5.1 `dedup` 의 `deep` 옵션 | **계획 3** | 폴더를 옮기기 전에 그 안의 중복을 걷어내려면 필요하다. `move_folders` 와 같이 다룬다 |
...
- Produces:
  ...
  - `organize.core.action.Action` — `kind`, `src: Path | None`, `dst: Path | None`, `reason: str`, `block: str`, `member: str | None = None`. frozen dataclass. `member` 는 `extract` 에서만 쓰며 압축 안의 원래 항목 이름을 담는다 (실행기가 무엇을 꺼낼지 알아야 한다). `size`/`mtime` 도 `extract` 전용으로, 압축 안에 적힌 값을 담는다 — `Context` 가 이 값으로 가상 엔트리를 만든다
...
- Produces:
  - `organize.core.paths.unique_path(dst: Path) -> Path` — 이미 있으면 `이름_(1).확장자`. **미리보기용이다** — 확인과 쓰기 사이에 틈이 있다
  - `organize.core.paths.claim_path(dst: Path) -> Path` — 빈 파일을 만들어 이름을 원자적으로 잡는다. **실제로 쓰기 직전에는 이걸 쓴다**
...
def claim_path(dst: Path) -> Path:
    ...
    재현했다 — 남의 파일 내용이 사라졌다. 미리보기용으로는 `unique_path` 가
    맞지만, **실제로 쓰기 직전에는 반드시 이 함수로 자리를 먼저 잡는다.**
    ...
    while True:
        ...
        try:
            ...
            raise OrganizeError(
                ...
                hint="대상 폴더의 쓰기 권한을 확인해 주세요.") from e
        os.close(fd)
        return candidate


def unique_path(dst: Path) -> Path:
    """미리보기용. 실제로 쓸 때는 `claim_path` 를 쓴다."""
    if not dst.exists():
        return dst
...
def same_drive(a: Path, b: Path) -> bool:
    ...
    답한다.** 이 답을 믿고 안전 검사를 건너뛰면 그 검사가 통째로 사라진다 —
    실제로 그런 결함이 있었다. `move_file` 은 이 함수를 쓰지 않는다.
    """
...
def move_file(src: Path, dst: Path) -> Path:
    ...
    try:
        ...
        raise                      # _move_onto 안에서 이미 정리하고 안내를 만들었다
    except BaseException:
        # Ctrl-C 나 예상 못 한 오류. 잡아 둔 빈 자리가 남으면 사용자 폴더에
        # 0바이트 파일이 생기고, 다음 실행 때 그 이름이 막혀 _(1) 이 밀린다.
...
**반드시 테스트할 것 — 미리보기 이름과 실제 이름이 어긋나는 경우.**
같은 이름을 두고 두 파일이 경합하면 미리보기(`unique_path`)와 실제 실행
(`claim_path`)의 결과가 갈릴 수 있다. 이 프로젝트의 약속은 "미리보기 ==
실제 실행" 인데, **이름 충돌은 그 약속에서 이미 예외로 받아들인 부분**이다
(디스크 상태는 미리보기 이후에도 바뀔 수 있으므로). 그래서 실행기가
`remap` 과 실행 로그의 `final` 로 실제 결과를 기록한다.
지켜야 할 선은 **파일이 사라지지 않는 것**이다. 두 파일이 같은 이름을 노려도
둘 다 남고 undo 가 둘 다 되돌리는지 반드시 테스트한다.

**반드시 확인할 것 — 잡아 둔 빈 자리의 잔해.**
`claim_path` 는 이름을 잡으려고 0바이트 파일을 만든다. `move_file` 은 모든
예외 경로와 Ctrl-C 에서 그걸 치우지만, **강제 종료(SIGKILL)는 막을 수 없다.**
그때 남는 0바이트 파일은 Task 19 의 `doctor` 가 찾아서 알려 주는 것이 맞다.
